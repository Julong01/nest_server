...
export class AuthService {
  ...
  async register(data: { loginId: string; password: string; email?: string }) {
    ...
    if (existing) throw new ConflictException('이미 가입된 아이디 또는 이메일입니다.');
  ...
  async login(loginId: string, password: string) {
    ...
    if (!user || !user.passwordHash) throw new UnauthorizedException('아이디 또는 비밀번호가 올바르지 않습니다.');
    const isMatch = await bcrypt.compare(password, user.passwordHash);
    if (!isMatch) throw new UnauthorizedException('아이디 또는 비밀번호가 올바르지 않습니다.');
  ...
  async refresh(refreshToken: string) {
    ...
    if (!token || token.revoked || token.expiresAt < new Date()) {
      throw new HttpException({ code: 'A401', message: '유효하지 않은 리프레시 토큰입니다.' }, 401);
    }
    const user = await prisma.user.findUnique({ where: { id: token.userId } });
    if (!user) {
      throw new HttpException({ code: 'A404', message: '사용자를 찾을 수 없습니다.' }, 404);
    }
    const payload = { sub: user.id, loginId: user.loginId };
    const accessToken = this.jwtService.sign(payload);
    return { accessToken };
